...
public class SideMenuViewImpl implements SideMenuView {
    ...
    private void init() {
        ...
        DomUtil.addHandler(navBtn, (ClickHandler) mouseEvent -> {
            if (collapsedToggle) {
                navBtn.getClassList().remove(collapsedStyle);
                navbarPanel.getClassList().add(showNavBarPanelStyle);
            } else {
                navBtn.getClassList().add(collapsedStyle);
                navbarPanel.getClassList().remove(showNavBarPanelStyle);
            }
            collapsedToggle = !collapsedToggle;
        });
        tree.asElement().getStyle().setProperty("padding-bottom", "80px");
    }
    ...
    public void setHeight(String height) {
        treePanel.getStyle().setProperty("max-height", height);
    }

    @Override
    public void setWidth(String width) {
        root.getStyle().setProperty("min-width", width);
    }
