...
public class TreeCellViewImpl extends BaseCellViewImpl implements BaseTreeCellView {
    ...
    protected void initDnd() {
        asElement().addEventListener("dragstart", new EventListener() {
            ...
            public void handleEvent(Event evt) {
                draggedElement = DomUtil.cast(evt.getTarget());
            }
        });
        ...
        asElement().addEventListener("dragleave", new EventListener() {
            @Override
            public void handleEvent(Event evt) {
                HTMLElement el = DomUtil.cast(evt.getTarget());
                el.getStyle().setProperty("border", "unset");
            }
        });
        asElement().addEventListener("dragover", new EventListener() {
            @Override
            public void handleEvent(Event evt) {
                MouseEvent event = DomUtil.cast(evt);
                HTMLElement el = asElement();
                if (draggedElement.equals(el)) {
                    return;
                }
                final double v = el.getBoundingClientRect().getBottom() - event.getClientY();
                final double vv = event.getClientY() - el.getBoundingClientRect().getTop();

                if (v < 10) {
                    el.getStyle().setProperty("border", "unset");
                    el.getStyle().setProperty("border-bottom", "2px solid red");
                    inserted = true;
                } else {
                    el.getStyle().setProperty("border", "2px red dotted");
                    inserted = false;
                }
                if (vv < 10) {
                    // Find the first <li> element with the "draggable" attribute within the parent <ul>
                    HTMLElement firstDraggableElement = ((HTMLElement) asElement().getParentNode()).querySelector("li[draggable]");
                    // Check if the element is the first draggable element
                    boolean isFirstDraggable = el.equals(firstDraggableElement);
                    if (isFirstDraggable) {
                        el.getStyle().setProperty("border", "unset");
                        el.getStyle().setProperty("border-top", "2px solid red");
                        inserted = true;
                        isFirstElement = true;
                    }
                }
                Console.log(el.getTextContent() + "  -  " + v);
                evt.preventDefault();
            }
        });
        ...
        asElement().addEventListener("drop", new EventListener() {
            @Override
            public void handleEvent(Event evt) {
                dragged.accept(draggedElement, isFirstElement ? null : DomUtil.cast(evt.getTarget()), inserted);
            }
        });
    }
    ...
    public void setActive(boolean b) {
        if (b) asElement().getClassList().add(activeStyle);
        else asElement().getClassList().remove(activeStyle);
    }
