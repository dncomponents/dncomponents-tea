        ...
        EventListener listener = new EventListener() {
            ...
            public void handleEvent(Event evt) {
                for (BaseCell baseCell : getCells()) {
                    if (DomUtil.contains(baseCell.asElement(), (Node) evt.getTarget())) {
...
//        return Collections.unmodifiableList(rows);
    ...
    public BaseCell<T, ?> getCell(HTMLElement element) {
        for (BaseCell baseCell : getCells()) {
            if (DomUtil.contains(baseCell.asElement(), element)) {
                return baseCell;
            }
        }
        return null;
...
//                .orElse(o -> true);
    ...
    public static void resetScrollOnPage(HTMLElement el) {
        NodeList<? extends HTMLElement> childNodes = el.querySelectorAll("*");
        for (int i = 0; i < childNodes.getLength(); i++) {
            final Object o = BaseComponent.allComponents.get(childNodes.item(i));
