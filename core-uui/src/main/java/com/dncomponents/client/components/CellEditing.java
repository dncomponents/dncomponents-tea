...
public class CellEditing<T, M> {
    ...
    private void onBlurEvent() {
        DomUtil.removeElement(asElement(cellEditor));
    ...
    private void onKeyUpEvent(KeyboardEvent event) {
        stopBlur = true;
        if (event.getCode().equals("Escape") && fromRow) {
            onBlurEvent();
        }
        stopBlur = false;
    }
    ...
    void addKeyUpHandlerToEditComponent(KeyUpHandler handler) {
        asElement(cellEditor).addEventListener(KeyUpHandler.TYPE, handler.asEventListener());
    }
    ...
    private HTMLElement asElement(CellEditor cellEdit) {
