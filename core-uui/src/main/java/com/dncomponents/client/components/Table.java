...
public class Table<T> extends AbstractCellComponent<T, List, TableUi> implements HasRowsDataList<T>, HasRowValueChangedHandlers<T>, HasRowEditingStoppedHandlers<T>, HasRowEditingStartedHandlers<T> {
    ...
    protected void addHeader() {
        ...
        if (headerRenderer != null) {
            view.getRootView().getHeaderRow().setInnerHTML("");
    ...
    protected void addFooter() {
        ...
        if (footerRenderer != null) {
            view.getRootView().getFooterRow().setInnerHTML("");
    ...
    public HandlerRegistration addRowHandler(BaseEventListener handler) {
        org.teavm.jso.dom.events.EventListener listener = new EventListener() {
            @Override
            public void handleEvent(Event evt) {
//                DomGlobal.console.log(evt.getTarget());
                for (RowTable<T> rowTable : getCells()) {
                    if (rowTable.asElement() == evt.getTarget() || DomUtil.contains(rowTable.asElement(), (Node) evt.getTarget())) {
    ...
    public HandlerRegistration addCellHandler(BaseEventListener handler) {
        org.teavm.jso.dom.events.EventListener listener = evt -> {
            if (getCell(evt) != null)
                handler.handleEvent(evt);
        };
        asElement().addEventListener(handler.getType(), listener);
    ...
    public RowTable<T> getRowCell(Event event) {
        for (RowTable<T> cell : getCells()) {
            if (cell.asElement() == event.getTarget() || DomUtil.contains(cell.asElement(), ((Node) event.getTarget()))) {
                return cell;
            }
        }
        return null;
    ...
    public TableCell<T, ?> getCell(Event evt) {
        for (RowTable<T> tRowTable : getCells()) {
            for (TableCell tableCell : tRowTable.getCells()) {
                if (DomUtil.contains(tableCell.asElement(), ((Node) evt.getTarget()))) {
                    return tableCell;
                }
            }
        }
    ...
    public static class TableHtmlParser extends AbstractPluginHelper implements ComponentHtmlParser {
        ...
        @Override
        public BaseComponent parse(HTMLElement htmlElement, Map<String, ?> templateElement) {
            ...
            if (htmlElement.hasChildNodes()) {
                ...
                NodeList<? extends HTMLElement> header_tags = htmlElement.getElementsByTagName(HEADER_TAG);
                for (int i = 0; i < header_tags.getLength(); i++) {
                    parseHeader((HTMLElement) header_tags.item(i), headerItems);
                    break; //we expect only one row.
                }

                NodeList<? extends HTMLElement> tags = htmlElement.getElementsByTagName(ROW_TAG);
                for (int i = 0; i < tags.getLength(); i++)
                    parseRow((HTMLElement) tags.item(i), rowItems);
                ...
                for (int i = 0; i < headerItems.size(); i++) {
                    ...
                    cellConfig.setCellFactory(c -> new TableCell<RowItemId, ItemId>().setRenderer(r ->
                            r.valuePanel.setInnerHTML(r.value.getContent()))
                    );
        ...
        private void parseHeader(HTMLElement html, List<ItemId> headerItems) {
            NodeList<? extends HTMLElement> tags = html.getElementsByTagName(CELL_TAG);
            for (int i = 0; i < tags.getLength(); i++)
                headerItems.add(getIdItem(tags.item(i)));
        }
        ...
        private void parseRow(HTMLElement html, List<RowItemId> rowIdItems) {
            ...
            for (int i = 0; i < tags.getLength(); i++)
                list.add(getIdItem(tags.item(i)));
