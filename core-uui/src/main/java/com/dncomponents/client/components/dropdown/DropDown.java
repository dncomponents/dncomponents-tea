        ...
        view.getRootView().addClickOnButton((ClickHandler) mouseEvent -> {
            ...
            if (menuVisible) {
                clickOutHandlerRegistration = view.getRootView().addClickOutOfButton(evt -> {
                    if (!DomUtil.isDescendant(this.asElement(), ((HTMLElement) evt.getTarget()))) {
    ...
    public static class DropDownHtmlParser extends AbstractPluginHelper implements ComponentHtmlParser {
        ...
        public BaseComponent parse(HTMLElement htmlElement, Map<String, ?> elements) {
            ...
            for (int i = 0; i < elementsByTagName.getLength(); i++) {
                DropDownItem<ItemId> dropDownItem = parseItem((HTMLElement) elementsByTagName.item(i), dropDown);
