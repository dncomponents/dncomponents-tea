...
public class Popover<T> extends BaseTooltip<T, PopoverView> implements HasCloseHandlers<Popover<T>>, HasOpenHandlers<Popover<T>> {
    ...
    public static class PopoverHtmlParser extends AbstractPluginHelper implements ComponentHtmlParser {
        ...
        @Override
        public Popover parse(HTMLElement html, Map<String, ?> templateElement) {
            ...
            NodeList<? extends HTMLElement> titles = html.getElementsByTagName("title");
            for (int i = 0; i < titles.getLength(); i++) {
                popover.getViewSlots().getTitle().setInnerHTML(titles.item(i).getTextContent());
                break;
            }
            NodeList<? extends HTMLElement> contents = html.getElementsByTagName("content");
            for (int i = 0; i < contents.getLength(); i++) {
                popover.getViewSlots().getContent().setInnerHTML(contents.item(i).getInnerHTML());
                break;
            }
            ...
            html.getParentNode().appendChild(toReplace);
    ...
    public static class PopoverAfterHtmlParser extends AbstractPluginHelper implements HtmlParser<String> {
        ...
        @Override
        public String parse(HTMLElement htmlElement1, Map<String, ?> template) {
            HTMLElement htmlElement = (HTMLElement) htmlElement1.getParentNode();
            if (htmlElement.hasAttribute("owner")) {
                ...
                DomUtil.removeElement(popover.asElement());
            ...
            DomUtil.removeElement(htmlElement);
            return null;
        }
