...
public class TableCellDate<T> extends TableCell<T, Date> {
    ...
    private void init() {
        ...
        setRenderer(r -> {
            if (r.value == null)
                r.valuePanel.setInnerHTML("N/D");
            else {
                final String s = JSDate.create(((double) value.getTime())).toLocaleDateString();
                r.valuePanel.setInnerHTML(JSDate.create(((double) value.getTime())).toLocaleDateString());
            }
        });
        //default cell editor
