...
public abstract class HtmlBinder<T> {
    ...
    public static void cssDevMode() {
        ...
        if (!allCss.isEmpty()) {
            var styleElement = HTMLDocument.current().createElement("style");
            // Set the CSS text
            styleElement.setTextContent(allCss.stream().collect(Collectors.joining("\n")));
            // Append the <style> element to the <head> of the document

            HTMLDocument.current().getHead().appendChild(styleElement);
        }
    }

    private static void removeCssFile(String href) {
        // Find the existing link element with the given href
        var linkElements = HTMLDocument.current().querySelectorAll("link[rel='stylesheet'][href='" + href + "']");
        // Remove each found link element
        for (int i = 0; i < linkElements.getLength(); i++) {
            HTMLElement element = linkElements.item(i);
