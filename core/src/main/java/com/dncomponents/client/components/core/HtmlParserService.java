...
public class HtmlParserService {
    ...
    public static void registerComponent(IsElementHtmlParser parser) {
        if (COMPONENT_HTML_PARSERS.stream().anyMatch(isElementHtmlParser -> isElementHtmlParser.getId().equals(parser.getId()))) {
            throw new IllegalStateException("Found duplicate tag " + parser.getId());
        }
    ...
    private static boolean isParserTag(Element element, List<? extends HtmlParser> parsers) {
        for (HtmlParser componentHtmlParser : parsers) {
            if (componentHtmlParser.getId().equalsIgnoreCase(element.getTagName())) {
                return true;
            }
        }
        return false;
