...
public interface IsElementHtmlParser<T extends IsElement> extends HtmlParser<T> {
    ...
    static void addClass(Element element1, HTMLElement element2) {
        ...
        if (style != null) {
            if (style.contains(" ")) {
                ...
                for (String word : words) {
                    if (!word.isEmpty())
                        element2.getClassList().add(word);
                }
            } else {
                element2.getClassList().add(style);
            }
    ...
    default T replaceAndCopy(Element element1, T element2) {
        ...
        element1.getParentNode().replaceChild(element2.asElement(), element1);
